...
- 设置页字体卡片固定为两列，卡片高度由 104 vp 缩短为 52 vp，字体名称字号由 14 fp 缩小为 10 fp。
- 字体保存成功后通过页面回调直接更新 `TodayViewModel`，不再依赖根页面返回时可能不触发的 `onPageShow`。
- 更新当天字体时保留现有正文、备注与背景草稿；提交与修订时从更新后的字体状态生成快照。
- 调试日志覆盖字体资源注册、设置加载与保存、主页接收、编辑器出现及提交快照，统一使用 `OneWord` 标签和 `font.` 前缀。
